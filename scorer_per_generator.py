...
def evaluate(pred_fpath, gold_fpath):
  ...
  gold_labels = pd.read_json(gold_fpath, lines=True)[['id', 'label', 'model']]
  ...
  for model in unique_models:
    ...
    model_accuracy = accuracy_score(model_df['label_gold'], model_df['label_pred'])
    model_accuracies[model] = model_accuracy
  
  return macro_f1, micro_f1, accuracy, model_accuracies
...
if __name__ == '__main__':
  ...
  if validate_files(pred_file_path):
    ...
    macro_f1, micro_f1, accuracy, model_accuracies = evaluate(pred_file_path, gold_file_path)
    logging.info("macro-F1={:.5f}\tmicro-F1={:.5f}\taccuracy={:.5f}".format(macro_f1, micro_f1, accuracy))
    logging.info(f"accuracy per generator: {model_accuracies}")
