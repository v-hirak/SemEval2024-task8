...
import evaluate
import numpy as np
from transformers import AutoModelForCausalLM, AutoModelForSequenceClassification, AutoModelForSeq2SeqLM, TrainingArguments, Trainer, DataCollatorWithPadding, AutoTokenizer, set_seed
import os
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dev_file_path", "-t", required=True, help="Path to the dev file.", type=str)
    ...
    parser.add_argument("--batch_size", "-b", required=False, default=16, help="Batch size.", type=int)
    parser.add_argument("--test_file_path", "-f", required=True, help="Path to the test file.", type=str)
    parser.add_argument("--test_prediction_save", "-f_save", required=True, help="Path to save test prediction.", type=str)
    
    args = parser.parse_args()

    random_seed = 0
    train_path =  args.train_file_path # For example 'subtaskA_train_multilingual.jsonl'
    test_path =  args.dev_file_path # For example 'subtaskA_test_multilingual.jsonl'
    ...
    tokenizer = AutoTokenizer.from_pretrained(f"{model}/subtask{subtask}/{random_seed}/best/")
    if model != "ai-forever/mGPT":
        model = AutoModelForSequenceClassification.from_pretrained(
           f"{model}/subtask{subtask}/{random_seed}/best/", num_labels=len(label2id), id2label=id2label, label2id=label2id    # put your model here
        )
    else:
        model = AutoModelForCausalLM.from_pretrained(
           f"{model}/subtask{subtask}/{random_seed}/best/", num_labels=len(label2id), id2label=id2label, label2id=label2id    # put your model here
    ...
    final_data = pd.read_json(args.test_file_path, lines=True)
    final_dataset = Dataset.from_pandas(final_data)

    tokenized_test_dataset = final_dataset.map(preprocess_function, batched=True,  fn_kwargs={'tokenizer': tokenizer})
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer)
    ...
    final_pred = pd.DataFrame({'id': final_data['id'], 'label': preds})
    final_pred.to_json(args.test_prediction_save, lines=True, orient='records')
